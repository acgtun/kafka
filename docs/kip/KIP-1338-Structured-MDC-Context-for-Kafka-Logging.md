...
### Container and Operator Deployments
- **Strimzi / Kubernetes**: Users can enable JSON logging by setting the `log4j2.configurationFile` system property via the `spec.kafka.jvmOptions.javaSystemProperties` or by mounting a custom `log4j2.yaml` in the Kafka pods. The new JSON config files should be included in Strimzi's Kafka image or provided via a ConfigMap.
- **Helm / docker-compose**: Set `KAFKA_LOG4J_OPTS=-Dlog4j2.configurationFile=file:/path/to/log4j2-json.yaml` in the environment.
- **Log shipping and parsing**: Existing log shippers (Fluent Bit, Filebeat, Vector, Promtail) will need updated parsers to extract the new top-level `kafka.*` fields. The JSON schema is stable and documented in `log4j2-json-template.json`.

### Performance in Production
The additional young-gen allocation is bounded by the rate of *enabled* log statements from context-bearing loggers. In practice this is dominated by INFO/WARN/ERROR logs from `UnifiedLog`, request handlers, and client internals — typically a few thousand events per second even on busy brokers. This is negligible compared with normal message processing throughput.

### Log Redaction and Compliance
Structured fields such as `kafka.client.id`, `kafka.transactional.id`, `kafka.group.id`, and `kafka.group.instance.id` will now appear in machine-readable form. Organizations with strict data retention or PII policies should ensure their log redaction pipelines are updated to handle these fields (they already existed in text prefixes).

## Future Work

- **Phase 2**: Extend MDC context to additional server components (`ReplicaManager`, `KafkaApis`, `Partition`, fetchers, group/transaction coordinators) and populate `kafka.cluster.id`, `kafka.connector`, and `kafka.task.id`.
- **Phase 3**: Instrument the Scala `Logging` trait so that legacy Scala code (still the majority of the broker) also emits structured context.
- Add official JMH benchmarks in `jmh-benchmarks/` for the logging hot path.
- Explore SLF4J 2.x / Log4j2 `ScopedValue` support for future virtual-thread adoption.

## Test Plan

### Implemented (in this KIP)

- 18 unit tests in `LogContextTest` covering:
  - Correct push/pop of MDC values
  - Preservation of pre-existing MDC entries (Kafka Connect compatibility)
  - Reentrancy and concurrent thread safety
  - Zero overhead when context map is empty or log level is disabled
  - Immutability of the context map
  - Backward compatibility of the single-argument constructor

- Manual verification that text logging remains unchanged when using default `log4j2.yaml`.
- Verification that the provided `log4j2-json*.yaml` files produce valid JSON containing the expected `kafka.*` fields.

### Planned / Recommended for Follow-up

- End-to-end integration test that starts a broker with `log4j2-json.yaml` and asserts the presence and correctness of structured fields in the JSON output.
- JMH benchmark comparing throughput and allocation rate with and without context maps.
- Chaos test exercising log level changes and async appenders while producing load.
